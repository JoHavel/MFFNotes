...
% 4. 1. 2021
    ...
    \begin{definice}[pdfTeX]
        \verb|\pdfoutput=0/1| registr, zda se má tisknout do DVI / PDF. Pak jsou registry na nastavení stránek. Pak můžeme nastavit verzi 1.x pomocí \verb|\pdfminorversion=x|. Pak existují \verb|\pdfcompresslevel=0-9| (jak moc se má komprimovat) a \verb|\pdfobjcompresslevel=0-3| (co všechno se má komprimovat).

        Registry \verb|\pdfpkresolution| a \verb|\pdfpkmode| nastavují převod bitmapových fontů (.pk). Vektorové fonty fungují podobně jako v dvips (\verb|\pdfmapfile{…}|, kde … je množina fontů s + a -, aby se daly nějak pospojovat, \verb|\pdfmapline{…}| řeší přímo nastavení fontu). Dále můžeme přidat tabulku ToUnicode: \verb|\pdfgentounicode=1| a pak pomocí \verb|\pdfglyphtounicode{PS jméno}{codepoiinty}| přidáváme záznamy (předpokládá, že PS font má a má různá jména) (dá se importovat \verb|\input glyphoutcode.tex|).

        FormXObject (libovolný objekt v PDF, který je uložen?): \verb|\setbox N=\hbox\{…\}| vytvoří objekt, \verb|\pdfxform N| zpracuje objekt (jeho pdf číslo \verb|O| pak lze získat \verb|\pdflastxform|) a potom se vkládá \verb|\pdfrefxform O|. Obdobně fungují obrázky (\verb|\pdfximage      [width X, height X, depth X][page N]{soubor}|, soubor může být z [PNG, JPEG, JBIG2, PDF], page -- vezmeme N tou stranu z např. PDF, pokud nespecifikujeme width / height, tak se roztáhne, pokud specifikujeme druhé, nebo se bere ze souboru, nebo z \verb|\pdfimageresolution|, pokud v souboru není, \verb|\pdfimageresolution=DPI|, \verb|\pdflastximage|, \verb|\pdfrefximage|, existuje jednotka px, aby se specifikoval rozměr v pixelech (rozměr px se pere z \verb|\pdfpxdimen|)).

        Barvy: \verb|\pdfcolorstackinit[page]{barva} -> N| (inicializuje stack a vrátí jeho číslo). Potom \verb|\pdfcolorstack N push {barva} / pop / set / current|. Barva je posloupnost PDF kreslících operátorů.

        Transformace se dají dělat pomocí barev ;), ale TeX by pak nevěděl, kde vykreslovat třeba odkazy. Proto se to dělá pomocí \verb|\pdfsetmatrix…|, \verb|\pdfsave|, \verb|\pdfrestore|
    \end{definice}

    \begin{definice}[Typografické featury pdfTeXu]
    ...
    \begin{definice}[Odkazy v pdfTeXu]
    ...
    \begin{definice}[Vertikální rozměry řádků]
    ...
    \begin{definice}[Ostatní featury]
