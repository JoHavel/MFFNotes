...
\begin{definice}[Vietoris topology]
...
\begin{tvrzeni}[Hausdorff metric]
...
% 18. 10. 2023 from text Deskriptivní teorie množin I

	\begin{dukazin}[Kompatibilita s Vietorisovou topologií]
...
\begin{definice}[Borel hierarchy]
...
\begin{definice}[Analytic set, coanalytic set]
...
\begin{poznamka}
	\ 
	\vspace{-2.5em}
	$$ A \in ∏_1^1(X), \quad ©K(A) = ©K(X) \setminus \{K \in ©K(X) | \exists x \in (X \setminus A) \cap K\} $$ % =: ©K(X) \setminus ©M$
...
\begin{definice}[Suslinovo schéma, Suslinova operace]
...
\begin{veta}[Szpilrajn-Marszewski (1907–1976)]
...
\begin{definice}[Univerzálně měřitelná]
...
\begin{veta}[Perfect set theorem]
...
\begin{definice}[Banach–Mazur game $G^{**}(M, Y)$]
...
\begin{veta}[From Real functions]
...
\begin{priklad}[„Closed“ is nessesary]
...
\begin{poznamka}[Historical remark]
