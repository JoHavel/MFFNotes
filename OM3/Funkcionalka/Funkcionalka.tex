...
%Na organizační úvod jsem nebyl.
...
\begin{definice}[Ekvivalentní normy]
...
\begin{lemma}
	...
	\begin{dukazin}
		...
		$\impliedby:$ Pokud $x = 0$, pak jsou nerovnosti jasné. Zvol $x≠0$. Pak $\frac{x}{||x||_1} \in B_1$. Pak $\frac{ax}{||x||_1} \in B_1 \subseteq B_2 \implies a||x||_2 ≤ ||x||_1$. Analogicky pro druhý směr.  
	\end{dukazin}
\end{lemma}
...
\begin{definice}[Konvexní množina]
...
\begin{definice}[Konvergence řady v normovaných lineárních prostorech]
...
\begin{veta}[Test úplnosti]
	...
	\begin{dukazin}
		...
		$\impliedby$: Ať $(x_n)$ je cauchyovská. Indukcí najdeme podposloupnost, že $||x_{n_k} - x_{n_{k+1}}|| < 2^{-k}$, $k \in ®N$. Pak
...
\begin{veta}
	...
	\begin{dukazin}[1.]
		...
		$\impliedby$: Ať $\sum_{\gamma \in \Gamma} x_\gamma$ splňuje B-C podmínku. Pak najdeme posloupnost $(F_n)_{n=1}^∞ \in ©F(\Gamma)^{®N}$, že
...
\begin{tvrzeni}
	...
	\begin{dukazin}
		...
		$\impliedby$: Ať $S:= \sup… < ∞$. Chceme $\sum_{\gamma \in \Gamma} a_\gamma = S$. Ať $\epsilon > 0$. Ať $H \in ©F(\Gamma)$ (z definice suprema) taková, že $S - \epsilon < \sum_{\gamma \in H} a_\gamma$. Pak pro $F' \supset H$ máme
...
\begin{tvrzeni}
	...
	\begin{dukazin}
		...
		$\impliedby$: Ať $\sum_{n \in ®N} ||x_n||$ je konvergentní, pak dle předchozího tvrzení
...
\begin{definice}[Izomorfismus, izomorfismus do, izometrie, izometrie do, izomorfní a izometrické prostory, izomorfně vnořený a izometricky vnořený prostor]
...
\begin{veta}
	...
	\begin{dukazin}
		...
		$\impliedby$: $\forall x \in X$: $||P_yx|| ≤ ||P_yx|| + ||P_z x|| = ||T x|| ≤ ||T||·||x||$, tedy $||P_y|| ≤ ||T||$.
	\end{dukazin}
\end{veta}
...
\begin{veta}
	...
	\begin{dukazin}
		$\impliedby$: Polož $p := T \circ S: X \rightarrow X$. Pak $p$ je zřejmě lineární a $||p|| ≤ ||T||·||S||$, navíc $p^2 = (T\circ S)\circ(T \circ S) = p$, tedy $p$ je projekce. Zároveň $p(X) = T(S(X))$, jelikož $S \circ T$ je identita, tak $S$ je na a $p(X) = T(Y) = \Rang T$. Zbývá si uvědomit, že $T$ je izomorfismus (izometrie, pokud $||S||, ||T|| ≤ 1$): Máme
...
\begin{veta}
	...
	\begin{dukazin}[Reálný případ, komplexní analogicky]
		$\implies$ z Polarizačního vzorce. Pro $\impliedby$ položme $\<x, y\> := \frac{1}{4}\(||x + y||^2 - ||x-y||^2\)$, $x, y \in X$. Následně ověříme podmínky (kromě linearity (speciálně aditivity) je ověření triviální). Aditivita: Chceme
...
\begin{veta}
	...
	\begin{dukazin}
		$\implies$ už víme. $\impliedby$: Víme $\sum_{n=1}^∞ x_n$ splňuje B-C podmínku. Tedy pro $\epsilon > 0\ \exists n_0 \in ®N$:
...
\begin{veta}[Vyjádření ortogonální projekce]
	...
	\begin{dukazin}
		% Z poznámek spolužačky, mockrát děkuji.
...
\begin{veta}[Hans Hahn (1927), Stefan Banach (1929)]
...
\begin{veta}[Oddělování konvexních množin]
...
% Tady byl ještě nějaký důsledek HB věty, ale nedospěl jsem k tomu, jaký (ani podle seznamu k přednášce ani podle skript). Podle fotky tabule jsem také nic nerozluštil.
...
\begin{veta}
	...
	\begin{dukazin}
		...
		$\impliedby$ (1.): Dle první části: $T^*$ je izomorfismus $\implies$ $T^{**}$ je izomorfismus $\implies$ $\epsilon_Y\circ T$ je izomorfismus $\implies$ $T$ je izomorfismus.
...
\begin{veta}
	...
	\begin{dukazin}
		1. $\impliedby$: Jasné protože pak $\Rang T \subset \spn\{y_1, …, y_n\}$. $\implies$: Ať $y_1, …, y_n$ je báze $\Rang T$. Uvažujme $g_i \in (\Rang T)^*$, $g_i(y_j) = \delta_{ij}$. Polož $f_i = g_i \circ T \in X^*$, $i \in [n]$. Pak $T(x) = \sum_{i=1}^n g_i(Tx)y_i = \sum_{i=1}^n f_i(x)y_i$.
	...
	\begin{dukazin}[Ze skript (vlastními slovy)]
...
\begin{veta}[J. P. Schauder, 1930]
	...
	\begin{dukazin}[Ze skript]
		$\impliedby$: Položme $K = \overline{T(B_X)}$ a $©F = \{f|_K | f \in B_{Y^*}\}$. Pak $K$ je kompaktní a $©F \subset C(K)$. Dále pro každé $f \in B_{Y^*}$ díky spojitosti $f$ platí
...
\begin{veta}[Princip stejnoměrné omezenosti]
	...
	\begin{dukazin}[Ze skript]
		$\implies$: Zřejmé. $\impliedby$: Pro $n \in ®N$ položme
...
\begin{tvrzeni}
	...
	\begin{dukazin}
		% Z poznámek spolužačky, moc děkuji.
		$\implies$: $(x^*)$ je omezená dle principu stejnoměrné omezenosti. $\impliedby$: Víme (aplikace linearity):
...
\begin{lemma}[J. P. Schauder, 1930]
	...
	\begin{dukazin}[První část ze skript]
...
\begin{dukaz}[Věty o otevřeném zobrazení]
...
\begin{dusledek}[S. Banach, 1929]
	...
	\begin{dukazin}
		„$\implies$“ jasné. „$\impliedby$“: $T^{-1}$ je spojitý plyne z předchozí věty ($(T^{-1})^{-1}(O) = T(O)$ je otevřené podle předchozí věty ($O$ otevřená)).
	\end{dukazin}
\end{dusledek}
...
\begin{veta}[O uzavřeném grafu (S. Banach, 1932)]
	...
	\begin{dukazin}
		„$\implies$“ trivální, platí vždy. „$\impliedby$“: $G:= \{(x, Tx) | x \in X\} \subset X \oplus_∞ Y$ je uzavřený (tedy Banachův). Ať $P_x$, $P_y$ jsou kanonické projekce (jsou spojité: $||P_x(x, y)|| = ||x||_x ≤ ||(x, y)||_∞$).
...
\begin{tvrzeni}
	...
	\begin{dukazin}
		...
		„$\impliedby$“: Plyne z důsledku výše.
	\end{dukazin}
\end{tvrzeni}
...
\begin{definice}[Vlastní číslo, vlastní prostor, vlastní vektory, bodové spektrum, spektrum operátoru]
...
\begin{veta}[Fredholmova alternativa]
	...
	\begin{dukazin}
		...
		„$\impliedby$“: $\lambda I - T$ je na $\implies$ (z nějaké předchozí věty) $(\lambda I - T)^* = \lambda I - T^*$ je prostý $\implies \lambda I - T^*$ je na $\implies \Ker (\lambda I - T) = (X^*)_\perp = \{0\} \implies \lambda I - T$ je prostý.
	\end{dukazin}
\end{veta}
...
\begin{veta}[Druhá Fredholmova věta]
...
\begin{veta}[Třetí Fredholmova věta]
...
\begin{definice}[Numerický range operátoru]
...
\begin{definice}[Samoadjungovaný operátor]
...
\begin{definice}[Invariantní množina]
...
\begin{veta}[Spektrální rozklad samoadjungovaného kompaktního operátoru (D. Hilbert (1904), Erhard Schmidt (1907))]
	...
	\begin{dukazin}[Jen za pomoci minulého lemmatu]
...
\begin{definice}[Regularizační jádro]
...
\begin{lemma}[Georg Friedrich Bernhard Riemann (1835), H. Lebesgue (1903)]
	...
	\begin{dukazin}
		...
		$$ = \frac{1}{2} ||\tau_0 f - \tau_{\pi \frac{t}{||t||^2}} f||_1 \rightarrow 0. $$
	\end{dukazin}
\end{lemma}

% 03. 01. 2022 Ze skript
...
\begin{veta}[O inverzi]
	...
	\begin{dukazin}[TODO vlastnosti $g$]
...
\begin{definice}[Schwartzův prostor]
...
\begin{lemma}
	...
	\begin{dukazin}[Na přednášce byl pouze náznak]
...
\begin{lemma}
	...
	\begin{dukazin}[Na přednášce pouze náznak]
