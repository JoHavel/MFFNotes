...
\begin{definice}[Invariantní zobrazení]
...
\begin{veta}
	...
	\begin{enumerate}
		...
		\item Nechť $y \in ®R^d$. Pak $\widehat{\tau_y f}(t) = e^{-\<y, t\>}\hat{f}(t)$ pro každé $t \in ®R^d$ a naopak pro funkci $h(x) = e^{i\<y, x\>}f(x)$ platí $\hat{h} = \tau_y \hat{f}$.
...
\begin{lemma}
	...
	\begin{dukazin}[Na přednášce pouze náznak]
		...
		$$ = \frac{1}{(2\pi)^{\frac{d}{2}}} \int_0^{+∞} \int_0^\pi … \int_0^\pi \int_0^{2\pi} \frac{r^{d-1}}{(1 + r^2)^{pN}} \sin^{d - 2} \phi_1 \sin^{d - 2} \phi_1 … \sin \phi_{d - 2} d\phi_{d - 2} … d\phi_1 dr = $$
