...
\begin{lemma}[J. P. Schauder, 1930]
	...
	\begin{dukazin}[První část ze skript]
		...
		Nechť tedy $r = s = 1$ a nechť je dáno $z \in U_Y$. Najdeme $\delta \in (0, 1)$ takové, že $||z|| < 1 - \delta$. Ukážeme, že $y = \frac{1}{1 - \delta}z \in T\(\frac{1}{1 - \delta} ©U_X\)$. Pak totiž $z = (1 - \delta)y \in (1 - \delta)T\(\frac{1}{1 - \delta}©U_X\) = T(©U_X)$.

		Pomocí matematické indukce najdeme $(y_n)_{n=0}^∞$ takové, že $y_0 = ¦o$, $||y - y_n|| < \delta^n$, $n \in ®N\cup\{0\}$ a $y_n - y_{n-1} \in T(\delta^{n-1} ©U_x)$:

		Je $||y|| < 1$, a tedy volbou $y_0 = ¦o$ je druhá podmínka splněna (a třetí se 0 netýká). Předpokládejme nyní, že $n \in ®N$ a již máme nalezeny prvky $y_0, …, y_{n-1}$. Pak
