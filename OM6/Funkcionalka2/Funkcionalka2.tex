...
\begin{lemma}
	...
	\begin{dukazin}
		...
		„Second item“: $x + h = x·(e + x^{-1}h)$ we have $x^{-1}$ exists and $(e + x^{-1}h)^{-1}$ exists (from first item), so $(x + h)^{-1}$ exists. Moreover
...
% 21. 02. 2023
%
	Pick $r > r(x)$. Claim: „$\frac{x^n}{r^n} \overset{w}\rightarrow 0$“: Fix $x^* \in A^*$ and put $f(\lambda) := \lambda · x^*(R_x(\lambda))$. By fact and tvrzeni after it, $f$ has derivative at each $\lambda \in \rho(x)$. Moreover for $|\lambda| ≥ \|x\|$ we have $f(\lambda) = \lambda · x^*\(\sum_{n=0}^∞ \frac{x^n}{\lambda^{n + 1}}\) = \sum_{n=0}^∞ \frac{x^*(x^n)}{\lambda^n}$. Thus $f(\lambda) = \sum_{n=0}^∞ \frac{x^*(x^n)}{\lambda^n}$, $\lambda \in P(0, r(x), ∞)$. From Complex analysis $f \in H(P(0, r, ∞))$ is uniquely given by Laurent series. In particular $f(r) = \sum_{n=0}^∞ \frac{x^*(x^n)}{r^n}$, so $x^*\(\frac{x^n}{r^n}\) \rightarrow 0$.

	From princip of unique boundedness (last semester): $\frac{x^n}{r^n}$ is $\|·\|$-bounded, so $\exists c > 0:$ $\|x^n\| ≤ c r^n$, $\sqrt[n]{\|x^n\|} ≤ \sqrt[n]{c}·r \rightarrow r$. So $\limsup \sqrt[n]{\|x^n\|} ≤ r$.
\end{dukaz}
...
\begin{veta}
	...
	\begin{dukazin}
		...
		„$\Phi$ is one-to-one“: Pick $\phi, \psi \in \Delta(A)$: $\Ker \phi = \Ker \psi$. Then (by lemma from previous semester) $\phi = c·\psi$ for some $c \in ®K$. But $\phi(e) = 1 = \psi(I)$ so $\phi = \psi$.
...
\begin{tvrzeni}[Polarization identity]
	...
	\begin{dukazin}[TODO!!!]
		($RHS = \frac{1}{4}(4·S(x, y) + 0·S(y, x) + 0·S(x, x) + 0·S(y, y))$.)
	\end{dukazin}
\end{tvrzeni}
...
\begin{dusledek}
	...
	\begin{dukazin}[TODO!!!]
		($\<Tx, x\> = \<Sx, x\> \implies \<Tx, y\> = \<Sx, y\> \implies S = T$)
	\end{dukazin}
\end{dusledek}
...
\begin{veta}
	...
	\begin{dukazin}[TODO!!!]
		($\<Tx, x\> \overset{\implies}= \<x, Tx\> = \overline{\<Tx, x\>} \overset{\impliedby}= \<Tx, x\>$.) ($\<Tx, Tx\> = \<T^*Tx, x\> = \<TT^*x, x\> = \<T^*x, T^*x\>$.) („$\implies$“: $ς(T) \subseteq \overline{N_T}$, „$\impliedby$“: $\inf N_T \in ς(T)$.)
	\end{dukazin}
\end{veta}
...
\begin{veta}[Hilbert–Schmidt]
	...
	\begin{dukazin}
		Omitted. „\verb|OM3/Funkcionalka.pdf|“
	\end{dukazin}
\end{veta}
...
\begin{veta}
	...
	\begin{dukazin}[TODO!!!]
		($4 \implies 1$ and $2 \implies 3$ was in the theorem above, $3 \implies 4$ trivial, $1 \implies 2$ obvious.)
	\end{dukazin}
\end{veta}
...
\begin{veta}[Polar decomposition]
